...
$\tolerance$, then opus's also does, so opus is feasible whenever sonnet is.

\subsection{Quality-loss tolerance}

A \emph{quality-loss tolerance} $\tolerance \in [0,1]$ is the maximum
...
\begin{equation}
  \theta_{\agent,\shape,\tier^*} - \theta_{\agent,\shape,\tier} \leq \tolerance,
  \label{eq:tolerance-constraint}
\end{equation}
i.e.\ the candidate tier $\tier$ may not lose more than $\tolerance$
absolute quality versus $\tier^*$. We partition $\tolerance$ into four
...
$\tolerance = 0$ (no downgrade ever).}
\label{tab:tolerance-tiers}
\begin{tabular}{lll}
\toprule
\textbf{Class} & \textbf{$\tolerance$} & \textbf{Typical workload} \\
\midrule
...
\begin{equation}
  \tier^\dagger
    = \arg\min_{\tier \in \tierset(\tolerance)}
      \cost(\tier),
  \label{eq:decision}
\end{equation}
where
\(
  \tierset(\tolerance) = \{ \tier : \theta^* - \theta_{\agent,\shape,\tier} \leq \tolerance \}
\)
is the feasible set under tolerance $\tolerance$. The decision rule is thus
``cheapest tier whose quality stays within tolerance.'' The advisor's job is
to estimate $\tierset(\tolerance)$ from data with high credibility.
...
\begin{equation}
  \tier^\dagger
    = \arg\min_{\tier \in \tierset_{\posterior, \tolerance, \alpha}}
      \cost(\tier),
...
\begin{equation}
  \tierset_{\posterior, \tolerance, \alpha}
    = \left\{ \tier : \Pr_{\theta \sim \posterior}\!\big[
        \theta^* - \theta_{\agent,\shape,\tier} \leq \tolerance
      \big] \geq 1 - \alpha
    \right\}
...
\(
  \mathrm{advise} : (\agent, \shape, \tolerance) \mapsto (\tier^\dagger,
