...
65\%-confidence cell maps to $\mathrm{Beta}(13, 7)$ (moderately
informative, mean $0.65$, standard deviation $\approx 0.10$); a never-evidenced cell maps to $\mathrm{Beta}(1, 1)$, the
uniform prior.
...
approximation suffices and is empirically competitive with full
hierarchical inference in our offline simulations.
...
tolerance $\qtol$, the conservative gate \emph{admits} $\tier$ if and
only if
\begin{equation}
...
$\qtol = 0$ (Critical) admits no downgrade ever, regardless of evidence
strength; $\qtol < 0.02$ (Strict) admits a downgrade only when the
conformal LCB exceeds baseline-mean by 2~percentage points; and so on
through Lenient.

\paragraph{Production approximation.}
The production implementation in \cite{german2026blackrim} computes
$\widehat{q}_{\mathrm{lo}}$ as a Wilson-style normal approximation to
the Beta posterior --- $\widehat{q}_{\mathrm{lo}} = \mu - z_{1-\alpha}
\sqrt{\sigma^2}$, where $\mu, \sigma^2$ are the Beta posterior moments
and $z_{1-\alpha}$ is the normal critical value --- because strict
conformal calibration requires a held-out exchangeable calibration set
that the early-deployment cells do not yet have. The two coincide
asymptotically as the calibration set grows; finite-sample coverage of
the Wilson approximation is liberal at low counts (we cap the lower
bound at $0$ for sparse cells, which the gate then rejects by default).
The supplied artefacts \cite{blackrim_paper_repo} use the same Wilson
LCB; a strict-conformal implementation backed by a rolling calibration
buffer is tracked separately (see \cref{sec:future}).
...
$L(\mathrm{down}, \mathrm{lost})$
  & $\Delta\cost_{\mathrm{down}} \cdot N_{\mathrm{dep}} \cdot M[\qtol]$
  & cascade \\
...
\begin{algorithm}[h]
\KwIn{Cell $(\agent, \shape)$, tolerance $\qtol$, baseline tier $\tier^*$,
...
\For{$\tier \in \tierset \setminus \{\tier^*\}$}{
  ...
  \If{$\tier \prec \tier^*$ \textbf{and} $\widehat{q}_{\mathrm{lo}}
...
\label{prop:regret}
In the unconstrained limit ($M = 0$, equivalently $\qtol \geq 1$, every
...
\begin{theorem}[Conservative property, asymptotic form]
\label{thm:conservative}
Suppose conformal calibration is exchangeable with the production stream
\cite{vovk2005algorithmic}, and the baseline-tier posterior has
concentrated so that $\mu^* \to \theta_{\agent,\shape,\tier^*}$
(equivalently, the number of baseline-tier observations $n^* \to \infty$).
Then for any cell $(\agent, \shape)$ and tolerance $\qtol$, CC-TS
recommends a downgrade $\tier \prec \tier^*$ only if
\begin{equation}
  \Pr\!\big[
    \theta_{\agent,\shape,\tier} \geq \theta_{\agent,\shape,\tier^*} - \qtol
  \big]
  \;\geq\;
  1 - \alpha.
  \label{eq:conservative}
\end{equation}
At finite $n^*$ the inequality carries an additional
$\mathcal{O}(1/\sqrt{n^*})$ error term from the
$\mu^* \mapsto \theta^*$ approximation in App~B; the production
implementation conservatively pads $\mu^*$ by one posterior standard
error at small $n^*$ to keep the inequality direction safe.
\end{theorem}

\begin{proof}[Proof sketch.]
The downgrade $\tier \prec \tier^*$ is admitted to $\mathrm{cands}$ in
\cref{alg:ccts} only if the conformal lower bound exceeds
$\mu^*(\agent, \shape) - \qtol$. By the marginal coverage guarantee of
