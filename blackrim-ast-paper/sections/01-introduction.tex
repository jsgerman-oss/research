...
We are also transparent about what data exist and what are pending.
\Cref{sec:eval} reports measured compression ratios (bench fixture),
token-budget conformance (Go corpus, Phase~1a), and outline emission
latency (Go corpus, Phase~1a). Three further claims --- adoption
trajectory, compile-gate false-negative rate, and end-to-end token
reduction on agent traces --- are pre-registered open questions
(OQ-AST-3 through OQ-AST-5) whose datasets are still accumulating.
The draft will be updated when those datasets fill; an interim
revision is appropriate once the adoption-trajectory and
end-to-end-reduction datasets are complete.
...
\begin{itemize}[leftmargin=*]
  ...
  \item \textbf{Empirical evaluation} (\cref{sec:eval}). We report
        cross-language compression ratios on a 4-file in-tree benchmark
        fixture (one representative file per language: Go 295~LoC,
        Python 253~LoC, JavaScript 185~LoC, TypeScript 188~LoC;
        measured savings: Go 82.0\%, Python 84.1\%, JavaScript 83.0\%,
        TypeScript 75.2\%) and a pre-registered empirical programme for
        the five further claims listed in \cref{sec:stance}.
\end{itemize}
